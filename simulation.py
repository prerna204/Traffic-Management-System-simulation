...
pygame.init()
pygame.font.init()
font_size = 25  # Change this value to make the text larger or smaller
font = pygame.font.Font(None, font_size)
white = (255, 255, 255)
black = (0, 0, 0)

steps = [
    "Step 1:Draw green boxes to measure traffic in pixels.",
    "Step 2: Convert pixels to meters using a pixel-to-meter model. ",
    "Step 3: Determine the green time based on the traffic length",
    # "Green Time = ","(Length of Traffic in Lane / Total Length of Traffic) * Total Signal Timer",
    "Step 4: Update the signal timer with the calculated green time"
]
steps1 = [
    "Lane  = pixel(px)   =  Length(m)  =  Green Time",
    "  L1   =   193.5px   =    60m      =       11sec",
    " L2   =   161.2px   =    50m      =       9sec",
    " L3   =   96.7px    =    30m      =       5sec",
    " L 4   =   128.2.5px    =    40m      =       7sec"

]
steps2 = [
    "Total Length: 60 + 50 + 30 + 40 = 180 meters",
    "Calculate Green Time for Lane 1:",
    "  Length (in pixels): 193.5 px",
    "  Converted Length (in meters): 193.5 × 0.3125 = 60.5 m",
    "  Green Time: (60.5 / 160) × 24 = 7 sec"
...
speeds = {'car': 1, 'bus': 1, 'truck': 1, 'bike': 1}  # average speeds of vehicles
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        path = "C:/Users/prern/PycharmProjects/pythonProject2/.venv/images/stimulation/" + direction + "/" + vehicleClass + ".png"
...
def initialize():
    ...
    if (randomGreenSignalTimer):
        ts1 = TrafficSignal(5, 2, green_times[0]+5)
        signals.append(ts1)
        ts2 = TrafficSignal(ts1.red-5 + ts1.green+ ts1.yellow, yellow_time, green_times[1])
        signals.append(ts2)
        ts3 = TrafficSignal(ts2.red + ts2.green, yellow_time, green_times[2])
        signals.append(ts3)
        ts4 = TrafficSignal(ts3.red + ts3.green, 4, green_times[3])
        signals.append(ts4)
    else:
        ts1 = TrafficSignal(5, 2, green_times[0])
        signals.append(ts1)
        ts2 = TrafficSignal(ts1.red - 5 + ts1.green + ts1.yellow, yellow_time, green_times[1])
        signals.append(ts2)
        ts3 = TrafficSignal(ts2.red + ts2.green, yellow_time, green_times[2])
        signals.append(ts3)
        ts4 = TrafficSignal(ts3.red + ts3.green, yellow_time, green_times[3])
        signals.append(ts4)
...
def calculate_signal_timing():
    ...
    for traffic_length in traffic_lengths:
        green_time = (traffic_length / total_traffic_length) * 36
...
def display_steps(screen):
    y_position = 20  # Starting y position for displaying steps
    x_position = 20  # Left side alignment
    for step in steps:
        text_surface = font.render(step, True, black, white)
        screen.blit(text_surface, (x_position, y_position))
        y_position += font_size + 10

def display_steps1(screen):
    y_position = 90  # Starting y position for displaying steps
    x_position = 900  # Left side alignment
    # y_position = 100  # Starting y position for displaying steps
    # x_position = 20 # Left side alignment
    for step in steps1:
        text_surface1 = font.render(step, True, black, white)
        screen.blit(text_surface1, (x_position, y_position))
        y_position += font_size + 10

def display_steps2(screen):
    y_position = 600  # Starting y position for displaying steps
    x_position = 20  # Left side alignment
    for step in steps2:
        text_surface2 = font.render(step, True, black, white)
        screen.blit(text_surface2, (x_position, y_position))
        y_position += font_size + 10
def initialize_random_vehicles():
    ...
    for direction in ['right', 'down', 'left', 'up']:
        ...
        for i in range(num_vehicles):
            ...
            vehicle.crossed = 0
def gather_all_vehicles(vehicles):
    all_vehicles = []
    for direction in ['right', 'down', 'left', 'up']:
        for lane in range(3):  # Assuming there are 3 lanes per direction
            if isinstance(vehicles[direction][lane], list):
                all_vehicles.extend(vehicles[direction][lane])
    return all_vehicles

def highlight_density_area(vehicles, screen, simulation_time, initial_red_time, ts4, threshold_distance_meters=5):
    ...
    for vehicle in vehicles:
        ...
        if hasattr(vehicle, 'image'):
            vehicle_rect = vehicle.image.get_rect(topleft=(vehicle.x, vehicle.y))

            if direction == 'right' and vehicle_rect.left <= defaultStop['right']:
            ...
            elif direction == 'left' and vehicle_rect.right >= defaultStop['left']:
            ...
            elif direction == 'down' and vehicle_rect.top <= defaultStop['down']:
            ...
            elif direction == 'up' and vehicle_rect.bottom >= defaultStop['up']:
    ...
    # Check if the bounding box should be displayed
    if simulation_time < initial_red_time or (simulation_time >50 and simulation_time<55):  # Use ts4 here
        for direction in directions:
            ...
            if bbox['min_x'] < bbox['max_x'] and bbox['min_y'] < bbox['max_y']:
                ...
                if simulation_time < initial_red_time or (simulation_time >50 and simulation_time<55) :
...
class Main:
    ...
    background = pygame.image.load('C:/Users/prern/PycharmProjects/pythonProject2/.venv/images/stimulation/intersection_f.png')

    screen = pygame.display.set_mode(screenSize)
    pygame.display.set_caption("SIMULATION")

    # Loading signal images and font
    redSignal = pygame.image.load('C:/Users/prern/PycharmProjects/pythonProject2/.venv/images/stimulation/red.png')
    yellowSignal = pygame.image.load('C:/Users/prern/PycharmProjects/pythonProject2/.venv/images/stimulation/yellow.png')
    greenSignal = pygame.image.load('C:/Users/prern/PycharmProjects/pythonProject2/.venv/images/stimulation/green.png')
    ...
    while True:
            ...
            for i in range(0,
                ...
                if (i == currentGreen):
                    ...
                    if (signals[i].red <=36):
                        ...
                        signals[i].signalText = 36-(signals[i].green)
                    screen.blit(redSignal, signalCoods[i])
        signalTexts = ["", "", "", ""]
        ...
        # y_position = 100 # Starting y position for lane-wise counts
        # meter =['60m', '50m','30m','40m']
        # for i in range(noOfSignals):
        #     laneText = font.render(f'Lane {i + 1}: {meter[i]}', True, black, white)
        #     screen.blit(laneText, (1100, y_position))
        #     y_position += 30
        ...
        display_steps1(screen)
        highlight_density_area(simulation, screen, simulation_time, initial_red_time, ts4)
